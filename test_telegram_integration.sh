#!/bin/bash
# ZeroBuild Telegram Integration Test Suite
...
if [ -f "target/release/zerobuild" ]; then
    BINARY_SIZE=$(ls -lh target/release/zerobuild | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/zerobuild 2>/dev/null || stat -c%s target/release/zerobuild)
...
CONFIG_PATH="$HOME/.zerobuild/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'zerobuild onboard' first"
    fi
else
    warn "No config file found - run 'zerobuild onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/zerobuild channel doctor 2>&1 || true)
END_TIME=$(date +%s)
...
6. [ ] Check logs for errors:
   RUST_LOG=debug zerobuild channel start
