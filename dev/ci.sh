...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    local build_args=(
      ...
      -t zerobuild-local-smoke:latest
      .
    )
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t zerobuild-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
ZeroBuild Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm zerobuild-local-smoke:latest --version
    ;;
