//! Performance benchmarks for ZeroBuild hot paths.
//!
//! Benchmarks cover:
...
use zerobuild::agent::agent::Agent;
use zerobuild::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use zerobuild::config::MemoryConfig;
use zerobuild::memory;
use zerobuild::memory::{Memory, MemoryCategory};
use zerobuild::observability::{NoopObserver, Observer};
use zerobuild::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use zerobuild::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "zerobuild architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "zerobuild"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about zerobuild agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("zerobuild agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
