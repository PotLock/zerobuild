...
/// <FunctionCall>
/// file_read
/// <code>path>/Users/kylelampa/Documents/zerobuild/README.md</code>
/// </FunctionCall>
/// ```
...
/// Map tool name aliases from various LLM providers to ZeroBuild tool names.
...
/// to. This function encodes the mapping for known ZeroBuild tools.
...
fn parse_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    // 2. ZeroBuild tool-call tags (<tool_call>, <toolcall>, <tool-call>)
...
/// Includes reasoning_content for thinking models (kimi-k2.5, Qwen3, etc.)
fn build_native_assistant_history(
    ...
    // CRITICAL: For thinking models like kimi-k2.5, reasoning_content MUST always be present
    // in assistant messages with tool_calls, even if empty. The API validates this field exists.
    let reasoning_val = reasoning_content
        .filter(|r| !r.is_empty())
...
pub(crate) async fn run_tool_call_loop(
    ...
    // Track consecutive failures per tool for escalation
    ...
    for iteration in 0..max_iterations {
        ...
        let (
            ...
            Ok(resp) => {
                ...
                // Also preserve reasoning_content for thinking models.
                let reasoning_content: Option<String> = resp.reasoning_content.clone();
                ...
                let assistant_history_content = if resp.tool_calls.is_empty() {
                    ...
                    build_native_assistant_history(&response_text, &resp.tool_calls, reasoning_ref)
                };
        ...
        if tool_calls.is_empty() {
            ...
            // For thinking models, we need to preserve reasoning_content in history
            // even when there are no tool calls. Build JSON structure if needed.
            let assistant_content = if let Some(ref reasoning) = reasoning_content {
                serde_json::json!({
            ...
            .zip(executed_outcomes.into_iter())
        {
            ...
            if outcome.success {
                ...
                *count += 1;
                ...
                // Inject escalation message after 3 consecutive failures
                if *count >= 3 {
                    let escalation_msg = "I'm having trouble with this step. Would you like me to try a different approach?";
                    tracing::warn!(
                        tool = %call.name,
                        failures = *count,
...
pub async fn run(
    ...
    // ── Load active project context for session resumption ────────
    let active_project = memory::load_project_context(mem.as_ref())
        .await
        .ok()
        .flatten();
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        zerobuild_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload agent-generated Arduino sketch. Use when: user asks for 'make a heart', 'blink pattern', or custom LED behavior on Arduino. You write the full .ino code; ZeroBuild compiles and uploads it. Pin 13 = built-in LED on Uno.",
        ));
        tool_descs.push((
            "hardware_memory_map",
    ...
    // Inject active project context into system prompt if available
    if let Some(ref project) = active_project {
        let project_context = memory::format_project_context_for_prompt(project);
    ...
    if let Some(msg) = message {
        ...
        // Show welcome message — different if resuming an active project
        if let Some(ref project) = active_project {
            println!("🦀 Welcome back to ZeroBuild!");
            println!(
                "You're building **{}**. Want to pick up where you left off?\n",
                project.name
            );
        } else {
            println!("🦀 ZeroBuild Interactive Mode");
...
pub async fn process_message(config: Config, message: &str) -> Result<String> {
    ...
    if let Some(task_type) = crate::agent::model_advisor::detect_task_type(message) {
        if !crate::agent::model_advisor::is_model_suitable(&model_name, task_type) {
            let has_routing = !config.model_routes.is_empty();
            let recommendation = crate::agent::model_advisor::generate_recommendation(
                &model_name,
                task_type,
                has_routing,
            );

            // Log recommendation (in interactive mode, this could be shown to user)
            tracing::info!("Model recommendation: {}", recommendation);

            // For now, add to memory so user can see it
            let _ = mem
                .store(
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload Arduino sketch. Use for 'make a heart', custom patterns. You write full .ino code; ZeroBuild uploads it.",
        ));
        tool_descs.push((
            "hardware_memory_map",
