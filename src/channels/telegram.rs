...
impl TelegramChannel {
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = zerobuild_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels_config.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `zerobuild onboard --channels-only` to configure interactively"
            );
        };
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to ZeroBuild now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Allowlist Telegram username (without '@') or numeric user ID.",
    ...
    async fn try_parse_attachment_message(
        ...
        // CRITICAL: Use numeric chat_id as sender for ZeroBuild create_job compatibility.
        // The chat_id is always numeric and matches Telegram's user/chat ID for API calls.
        Some(ChannelMessage {
...
Ensure only one `zerobuild` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@zerobuild_bot 654321"),
            Some("654321")
        );
    }
