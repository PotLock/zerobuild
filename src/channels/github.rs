    ...
    async fn post_issue_comment(
        ...
        for attempt in 1..=3 {
            let response = self
                ...
                .header("User-Agent", "ZeroBuild-GitHub-Channel")
                .json(&payload)
                .send()
                .await?;
...
mod tests {
    ...
    fn make_channel() -> GitHubChannel {
        GitHubChannel::new(
            "ghp_test".to_string(),
            None,
            vec!["louisdevzz/zerobuild".to_string()],
        )
    }
    ...
    fn parse_issue_comment_event_created() {
        ...
        let payload = serde_json::json!({
            "action": "created",
            "repository": { "full_name": "louisdevzz/zerobuild" },
            "issue": { "number": 2079, "title": "GitHub as a native channel" },
            "comment": {
                "id": 12345,
                "body": "please add this",
                "created_at": "2026-02-27T14:00:00Z",
                "html_url": "https://github.com/louisdevzz/zerobuild/issues/2079#issuecomment-12345",
                "user": { "login": "alice", "type": "User" }
            }
        });
        let msgs = ch.parse_webhook_payload("issue_comment", &payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].reply_target, "louisdevzz/zerobuild#2079");
    ...
    fn parse_pr_review_comment_event_created() {
        ...
        let payload = serde_json::json!({
            ...
            "comment": {
                ...
                "created_at": "2026-02-27T14:00:00Z",
                "html_url": "https://github.com/louisdevzz/zerobuild/pull/2118#discussion_r9001",
                "user": { "login": "bob", "type": "User" }
            }
        });
        let msgs = ch.parse_webhook_payload("pull_request_review_comment", &payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].reply_target, "louisdevzz/zerobuild#2118");
    ...
    fn parse_issue_recipient_format() {
        assert_eq!(
            GitHubChannel::parse_issue_recipient("louisdevzz/zerobuild#12"),
            Some(("louisdevzz/zerobuild", 12))
        );
