//! Flash ZeroBuild Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash ZeroBuild Nucleo firmware. Builds from firmware/zerobuild-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (zerobuild's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("zerobuild-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from zerobuild repo root.",
            firmware_dir.display()
        );
    }

    println!("Building ZeroBuild Nucleo firmware...");
    ...
    println!("ZeroBuild Nucleo firmware flashed successfully.");
