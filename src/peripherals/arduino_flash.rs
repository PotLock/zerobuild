//! Flash ZeroBuild Arduino firmware via arduino-cli.
//!
...
/// ZeroBuild Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/zerobuild-arduino/zerobuild-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "zerobuild-arduino";
...
/// Flash ZeroBuild firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("zerobuild_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling ZeroBuild Arduino firmware...");
    ...
    println!("ZeroBuild firmware flashed successfully.");
