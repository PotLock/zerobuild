...
/// `ZeroBuild` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "zerobuild")]
...
Examples:
  zerobuild agent                              # interactive session
  zerobuild agent -m \"Summarize today's logs\"  # single message
  zerobuild agent -p anthropic --model claude-sonnet-4-20250514
  zerobuild agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  zerobuild gateway                  # use config defaults
  zerobuild gateway -p 8080          # listen on port 8080
  zerobuild gateway --host 0.0.0.0   # bind to all interfaces
  zerobuild gateway -p 0             # random available port")]
    Gateway {
...
Launches the full ZeroBuild runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run ZeroBuild in \
production or as an always-on assistant.

Use 'zerobuild service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  zerobuild daemon                   # use config defaults
  zerobuild daemon -p 9090           # gateway on port 9090
  zerobuild daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  zerobuild cron list
  zerobuild cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  zerobuild cron add '*/30 * * * *' 'Check system health'
  zerobuild cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  zerobuild cron add-every 60000 'Ping heartbeat'
  zerobuild cron once 30m 'Run backup in 30 minutes'
  zerobuild cron pause <task-id>
  zerobuild cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect ZeroBuild \
...
Examples:
  ...
  zerobuild channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  zerobuild channel remove my-bot
  zerobuild channel bind-telegram zerobuild_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  zerobuild hardware introspect /dev/ttyACM0
  zerobuild hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: zerobuild::HardwareCommands,
    },
...
Examples:
  ...
  zerobuild peripheral add nucleo-f401re /dev/ttyACM0
  zerobuild peripheral add rpi-gpio native
  zerobuild peripheral flash --port /dev/cu.usbmodem12345
  zerobuild peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: zerobuild::PeripheralCommands,
    },
...
Examples:
  ...
  zerobuild memory list --category core --limit 10
  zerobuild memory get <key>
  zerobuild memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage ZeroBuild configuration.
...
Examples:
  zerobuild config schema              # print JSON Schema to stdout
  zerobuild config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `zerobuild`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(zerobuild completions bash)
  zerobuild completions zsh > ~/.zfunc/_zerobuild
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("ZEROBUILD_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("ZEROBUILD_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting ZeroBuild Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting ZeroBuild Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting ZeroBuild Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting ZeroBuild Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 ZeroBuild Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `zerobuild auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `zerobuild auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `zerobuild auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `zerobuild auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `zerobuild auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `zerobuild auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["zerobuild", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["zerobuild", "onboard", "--force"])
