...
mod tests {
    ...
    fn resolved_path_blocks_outside_workspace() {
        let workspace = std::env::temp_dir().join("zerobuild_test_resolved_path");
        ...
        let outside = canonical_temp.join("outside_workspace_zerobuild");
        assert!(
    ...
    fn resolved_path_blocks_root_escape() {
        let policy = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/zerobuild_user/project"),
            ..SecurityPolicy::default()
        };
    ...
    fn resolved_path_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("zerobuild_test_symlink_escape");
    ...
    fn allowed_roots_permits_paths_outside_workspace() {
        ...
        let root = std::env::temp_dir().join("zerobuild_test_allowed_roots");
        let workspace = root.join("workspace");
