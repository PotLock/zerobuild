...
impl SyscallAnomalyDetector {
    ...
    pub fn new(
        config: SyscallAnomalyConfig,
        zerobuild_dir: impl AsRef<Path>,
        audit_config: AuditConfig,
    ) -> Self {
        let baseline = normalize_baseline(&config.baseline_syscalls);
        let anomaly_log_path = resolve_log_path(zerobuild_dir.as_ref(), config.log_path.as_str());
        let audit_logger = AuditLogger::new(audit_config, zerobuild_dir.as_ref().to_path_buf()).ok();
