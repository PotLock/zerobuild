...
// in `~/.zerobuild/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.zerobuild/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(zerobuild_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: zerobuild_dir.join(".secret_key"),
            enabled,
        }
    }
