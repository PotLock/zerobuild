//! WASM plugin tool — executes a `.wasm` binary as a ZeroBuild tool.
//!
//! # Feature gate
...
pub struct WasmManifest {
    ...
    pub version: String,
    /// Optional homepage / source URL (shown in `zerobuild skill list`).
    #[serde(default)]
...
///
/// **Installed layout** (from `zerobuild skill install`):
/// ```text
/// skills/<skill-name>/tools/<tool-name>/tool.wasm
/// skills/<skill-name>/tools/<tool-name>/manifest.json
/// ```
///
/// **Dev layout** (direct from `zerobuild skill install ./my-tool`):
/// ```text
...
mod tests {
    ...
    fn load_from_empty_dir_returns_empty() {
        let tools = load_wasm_tools_from_skills(std::path::Path::new(
            "/tmp/zerobuild_wasm_test_nonexistent_xyz",
        ));
    ...
    fn manifest_load_from_missing_file_returns_error() {
        let result = WasmManifest::load_from(&PathBuf::from(
            "/nonexistent_zerobuild_test_dir/manifest.json",
        ));
    ...
    fn manifest_with_optional_fields_parsed() {
        let json = serde_json::json!({
            "name": "zerobuild_optional_test",
            "description": "Tool with all optional fields",
            "parameters": { "type": "object", "properties": {} },
            "version": "2",
            "homepage": "https://example.com/zerobuild_optional_test"
        });
        let m: WasmManifest = serde_json::from_value(json).unwrap();
        assert_eq!(m.version, "2");
        assert_eq!(
            m.homepage.as_deref(),
            Some("https://example.com/zerobuild_optional_test")
        );
    }
    ...
    fn load_wasm_tools_skips_dir_missing_manifest() {
        let dir = tempfile::tempdir().unwrap();
        let skill_dir = dir.path().join("zerobuild_test_skill");
    ...
    fn load_wasm_tools_installed_layout_skips_bad_manifest() {
        ...
        let tool_dir = dir
            .path()
            .join("zerobuild_test_pkg")
            .join("tools")
            .join("zerobuild_test_func");
    ...
    fn wasm_tool_load_rejects_missing_file() {
        let result = WasmTool::load(
            &PathBuf::from("/nonexistent_zerobuild_test_wasm/tool.wasm"),
            "zerobuild_missing_test".into(),
            "desc".into(),
            serde_json::json!({}),
        );
        assert!(result.is_err());
