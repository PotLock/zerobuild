...
struct NativeChatRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool_choice: Option<String>,
    /// Enable reasoning/thinking mode for models like kimi-k2.5
    /// Required for proper tool call support with thinking models
    #[serde(skip_serializing_if = "Option::is_none")]
...
struct NativeMessage {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool_calls: Option<Vec<ToolCall>>,
    /// Reasoning/thinking content for models like kimi-k2.5, Qwen3.
    /// Present when the model returned actual thinking content. Omitted when None
    /// to avoid sending null/empty values that some APIs (e.g. Kimi) reject.
    #[serde(skip_serializing_if = "Option::is_none")]
    reasoning_content: Option<String>,
}
                ...
                if message.role == "assistant" {
                    if let Ok(value) = serde_json::from_str::<serde_json::Value>(&message.content)
                    {
                        ...
                        if value.get("tool_calls").is_some() || value.get("reasoning_content").is_some() {
                            ...
                            // Extract reasoning_content for thinking models (kimi-k2.5, Qwen3, etc.)
                            // Only include when the stored value is actually non-empty — sending
                            // null or empty string causes Kimi to reject with "reasoning_content is missing".
...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn chat_with_system(
        ...
        let credential = self.credential.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "{} API key not set. Run `zerobuild onboard` or set the appropriate env var.",
                self.name
            )
        })?;
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are ZeroBuild".to_string()),
                },
                Message {
                    role: "user".to_string(),
