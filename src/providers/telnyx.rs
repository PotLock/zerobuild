...
///
/// ```rust,ignore
/// use zerobuild::providers::telnyx::TelnyxProvider;
/// use zerobuild::providers::Provider;
///
...
impl TelnyxProvider {
    ...
    /// 2. `ZEROBUILD_API_KEY` environment variable (fallback)
...
impl Provider for TelnyxProvider {
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `zerobuild onboard`."
            )
        })?;
