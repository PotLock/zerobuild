...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to ZeroBuild — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("ZEROBUILD_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to ZeroBuild)");
...
/// Use `zerobuild onboard` or `zerobuild onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `zerobuild onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".zerobuild");
...
async fn run_quick_setup_with_home(
    ...
    let (zerobuild_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        ...
        println!("    2. Or edit:           ~/.zerobuild/config.toml");
        println!("    3. Chat:              zerobuild agent -m \"Hello!\"");
        println!("    4. Gateway:           zerobuild gateway");
    } else {
        println!("    1. Chat:     zerobuild agent -m \"Hello!\"");
        println!("    2. Gateway:  zerobuild gateway");
        println!("    3. Status:   zerobuild status");
    }
    println!();
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `zerobuild models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
async fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    // Debug: Show which env vars are affecting the path
    if let Ok(val) = std::env::var("ZEROBUILD_CONFIG_DIR") {
        print_bullet(&format!(
            "{} ZEROBUILD_CONFIG_DIR={}",
            style("Note:").yellow(),
            style(&val).dim()
        ));
    }
    if let Ok(val) = std::env::var("ZEROBUILD_WORKSPACE") {
        print_bullet(&format!(
            "{} ZEROBUILD_WORKSPACE={}",
            style("Note:").yellow(),
            style(&val).dim()
    ...
    // Check for active_workspace.toml marker file
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let marker_file = home.join(".zerobuild").join("active_workspace.toml");
    if marker_file.exists() {
        if let Ok(content) = tokio::fs::read_to_string(&marker_file).await {
            print_bullet(&format!(
                "{} Found active_workspace.toml: {}",
                style("Note:").yellow(),
                style(content.trim()).dim()
            ));
        }
    }
    ...
    // Show hint if path looks like temp directory
    let path_str = default_workspace_dir.to_string_lossy();
    if path_str.contains("/var/folders/") || path_str.contains("/tmp/") {
        print_bullet(&format!(
            "{} Detected temp directory. To use ~/.zerobuild:",
            style("Hint:").cyan()
        ));
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("ZeroBuild will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how ZeroBuild connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("ZeroBuild uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("ZeroBuild can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    ...
    let choice = Select::new()
        .with_prompt("  How should ZeroBuild interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how ZeroBuild stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to ZeroBuild from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBuild from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBuild from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBuild from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("ZeroBuild reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.zerobuild/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("zerobuild-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via ZEROBUILD_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBuild from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("ZeroBuild will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("ZeroBuild is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("zerobuild auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: zerobuild auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("zerobuild channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("zerobuild agent -m \"Hello, ZeroBuild!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("zerobuild agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("zerobuild status").yellow());
...
mod tests {
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        tokio::fs::create_dir_all(&zerobuild_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        ...
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("zerobuild-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".zerobuild").join("config.toml");

        let _workspace_env = EnvVarGuard::set(
            "ZEROBUILD_WORKSPACE",
            workspace_dir.to_string_lossy().as_ref(),
        );
        let _config_env = EnvVarGuard::unset("ZEROBUILD_CONFIG_DIR");
        ...
        .await
        .expect("quick setup should honor ZEROBUILD_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** ZeroBuild"),
            "should default agent name to ZeroBuild"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("ZeroBuild-v2"));
    }
