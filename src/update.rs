//! Self-update functionality for ZeroBuild.
//!
...
const GITHUB_REPO: &str = "zerobuild-labs/zerobuild";
const GITHUB_API_RELEASES: &str =
    "https://api.github.com/repos/zerobuild-labs/zerobuild/releases/latest";
...
fn get_archive_name(target: &str) -> String {
    if target.contains("windows") {
        format!("zerobuild-{}.zip", target)
    } else {
        format!("zerobuild-{}.tar.gz", target)
    }
}
...
async fn fetch_latest_release() -> Result<Release> {
    let client = reqwest::Client::builder()
        .user_agent(format!("zerobuild/{}", current_version()))
        .build()
...
pub async fn self_update(force: bool, check_only: bool) -> Result<()> {
    ...
    if check_only {
        ...
        println!("Run `zerobuild update` to install the update.");
        return Ok(());
    }
    ...
    println!();
    println!("Restart ZeroBuild to use the new version.");
