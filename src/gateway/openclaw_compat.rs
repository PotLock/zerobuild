...
//!
//! Provides two endpoints for callers migrating from OpenClaw to ZeroBuild:
//!
//! 1. **`POST /api/chat`** (recommended) — ZeroBuild-native endpoint that invokes the
...
//! ZeroBuild's existing `/v1/chat/completions` (in `openai_compat.rs`) uses the
...
// ══════════════════════════════════════════════════════════════════════════════
// /api/chat — ZeroBuild-native endpoint
// ══════════════════════════════════════════════════════════════════════════════
...
pub struct ApiChatBody {
    ...
    /// Use this to inject recent conversation history that ZeroBuild's
...
/// `POST /v1/chat/completions` — OpenAI-compatible shim over ZeroBuild's agent loop.
///
...
/// the same tools + memory experience as native ZeroBuild channels.
