    ...
    match command {
        crate::SopCommands::List => {
            let sops = load_sops(
                ...
                sops_dir_override,
                config.sop.default_execution_mode.into(),
            );
            if sops.is_empty() {
                println!("No SOPs found.");
                println!();
                println!("  Create one: mkdir -p ~/.zerobuild/workspace/sops/my-sop");
        ...
        crate::SopCommands::Validate { name } => {
            ...
            let matching: Vec<&Sop> = if !name.is_empty() {
...
"#,
    ...
    fn resolve_sops_dir_default() {
        let ws = Path::new("/home/user/.zerobuild/workspace");
