...
const SERVICE_LABEL: &str = "com.zerobuild.daemon";
const WINDOWS_TASK_NAME: &str = "ZeroBuild Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "zerobuild.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["zerobuild", "start"]))?;
        }
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["zerobuild", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/zerobuild");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/zerobuild");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "zerobuild", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove zerobuild from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/zerobuild)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: zerobuild service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=ZeroBuild daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "zerobuild.service"]));
...
/// Check if the zerobuild user exists and has expected properties.
...
fn check_zerobuild_user() -> Result<()> {
    ...
    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser zerobuild && delgroup zerobuild",
            "addgroup -S zerobuild && adduser -S -s /sbin/nologin -H -D -G zerobuild zerobuild",
        )
    } else {
        ("userdel zerobuild", "useradd -r -s /sbin/nologin zerobuild")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'zerobuild' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'zerobuild' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/zerobuild" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: zerobuild user has home directory '{}' (expected /var/lib/zerobuild or /nonexistent)",
                        home
                    );
                }
...
fn ensure_zerobuild_user() -> Result<()> {
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "zerobuild"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "zerobuild"])
                .output()
                .context("Failed to create zerobuild group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create zerobuild group: {}", stderr.trim());
            }
            println!("✅ Created system group: zerobuild");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create zerobuild user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create zerobuild user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "zerobuild"])
            .output()
            .context("Failed to create zerobuild user")?;
    ...
    println!("✅ Created system user: zerobuild");
    Ok(())
}

/// Change ownership of a path to zerobuild:zerobuild
#[cfg(unix)]
fn chown_to_zerobuild(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["zerobuild:zerobuild", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to zerobuild:zerobuild: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_zerobuild(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_zerobuild(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "zerobuild:zerobuild", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to zerobuild:zerobuild: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_zerobuild(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".zerobuild"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'zerobuild' cannot write {} ({details}). \
             Re-run `sudo zerobuild service install` and ensure ownership is zerobuild:zerobuild.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/zerobuild",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="zerobuild:zerobuild"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/zerobuild/access.log"
error_log="/var/log/zerobuild/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/zerobuild");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo zerobuild service install"
        );
    }

    ensure_zerobuild_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/zerobuild");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/zerobuild");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_zerobuild(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by zerobuild:zerobuild)",
    ...
    chown_recursive_to_zerobuild(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/zerobuild");
    ...
    run_checked(Command::new("rc-update").args(["add", "zerobuild", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/zerobuild");
    println!("   Config path: /etc/zerobuild/config.toml");
    println!("   Start with: sudo zerobuild service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("zerobuild-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/zerobuild.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "ZeroBuild Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/zerobuild");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/zerobuild"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"zerobuild\""));
        assert!(script.contains("description=\"ZeroBuild daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/zerobuild\""));
        assert!(script.contains("command_args=\"--config-dir /etc/zerobuild daemon\""));
        assert!(!script.contains("env ZEROBUILD_CONFIG_DIR"));
        assert!(!script.contains("env ZEROBUILD_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"zerobuild:zerobuild\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/zerobuild/access.log\""));
        assert!(script.contains("error_log=\"/var/log/zerobuild/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/zerobuild");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/zerobuild");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/zerobuild");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/zerobuild"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/zerobuild'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/zerobuild/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/zerobuild/workspace'".to_string(),
                "zerobuild".to_string()
            ]
        );
    }
