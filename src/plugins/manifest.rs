//! Plugin manifest — the `zerobuild.plugin.toml` descriptor.
//!
//! Mirrors OpenClaw's `openclaw.plugin.json` but uses TOML to match
//! ZeroBuild's existing config format.
...
pub const PLUGIN_MANIFEST_FILENAME: &str = "zerobuild.plugin.toml";
...
/// Load and parse `zerobuild.plugin.toml` from `root_dir`.
