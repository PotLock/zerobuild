//! Plugin system for ZeroBuild.
//!
//! Modeled after OpenClaw's plugin architecture, adapted for Rust:
//!
//! - **Manifest**: each plugin has a `zerobuild.plugin.toml` descriptor
//! - **Discovery**: scans bundled, global (`~/.zerobuild/extensions/`), and
//!   workspace (`.zerobuild/extensions/`) directories
...
//!
//! ```rust,ignore
//! use zerobuild::plugins::{Plugin, PluginApi, PluginManifest};
//!
...
mod tests {
    ...
    fn module_reexports_are_accessible() {
        ...
        assert_eq!(PLUGIN_MANIFEST_FILENAME, "zerobuild.plugin.toml");
    }
}
