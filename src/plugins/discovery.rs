...
//! extension directories for subdirectories containing `zerobuild.plugin.toml`.
...
/// 2. Global: `~/.zerobuild/extensions/`
/// 3. Workspace: `<workspace>/.zerobuild/extensions/`
...
pub fn discover_plugins(workspace_dir: Option<&Path>, extra_paths: &[PathBuf]) -> DiscoveryResult {
    ...
    if let Some(home) = dirs_home() {
        let global = home.join(".zerobuild").join("extensions");
    ...
    if let Some(ws) = workspace_dir {
        let ws_ext = ws.join(".zerobuild").join("extensions");
...
"#
    ...
    fn discover_from_workspace() {
        ...
        let ext_dir = ws.join(".zerobuild").join("extensions");
