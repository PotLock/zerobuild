...
Examples:
  ...
  zerobuild channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  zerobuild channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `ZeroBuild` workspace
    Openclaw {
...
Examples:
  ...
  zerobuild cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  zerobuild cron add-every 60000 'Ping heartbeat'     # every minute
  zerobuild cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  zerobuild cron once 2h 'Follow up on deployment'
  zerobuild cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  zerobuild cron update <task-id> --expression '0 8 * * *'
  zerobuild cron update <task-id> --tz Europe/London --name 'Morning check'
  zerobuild cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  zerobuild hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  zerobuild peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash ZeroBuild firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash ZeroBuild firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  zerobuild peripheral flash
  zerobuild peripheral flash --port /dev/cu.usbmodem12345
  zerobuild peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash ZeroBuild firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
...
pub enum SopCommands {
    ...
    List,
    /// Validate a SOP definition
    Validate {
        /// SOP name to validate
        name: String,
    },
