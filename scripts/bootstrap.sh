...
usage() {
  cat <<'USAGE'
ZeroBuild installer bootstrap engine

Usage:
  ./zerobuild_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds ZeroBuild only (requires existing Rust toolchain).
...
Examples:
  ...
  ./zerobuild_install.sh --guided
  ./zerobuild_install.sh --install-system-deps --install-rust
  ./zerobuild_install.sh --prefer-prebuilt
  ./zerobuild_install.sh --prebuilt-only
  ./zerobuild_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./zerobuild_install.sh --interactive-onboard
...
Environment:
  ZEROBUILD_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  ZEROBUILD_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  ZEROBUILD_DOCKER_IMAGE      Docker image tag to build/run (default: zerobuild-bootstrap:local)
  ZEROBUILD_API_KEY           Used when --api-key is not provided
  ZEROBUILD_PROVIDER          Used when --provider is not provided (default: openrouter)
  ZEROBUILD_MODEL             Used when --model is not provided
  ZEROBUILD_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  ZEROBUILD_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  ZEROBUILD_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${ZEROBUILD_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${ZEROBUILD_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  extracted_bin="$temp_dir/zerobuild"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name zerobuild -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable zerobuild binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/zerobuild"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/zerobuild"
...
run_guided_installer() {
  ...
  echo
  echo "ZeroBuild guided installer"
  ...
  if prompt_yes_no "Install zerobuild into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${ZEROBUILD_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set ZEROBUILD_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set ZEROBUILD_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${ZEROBUILD_DOCKER_IMAGE:-zerobuild-bootstrap:local}"
  fallback_image="ghcr.io/zerobuild-labs/zerobuild:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.zerobuild-docker"
  else
    default_data_dir="$WORK_DIR/.zerobuild-docker"
  fi
  docker_data_dir="${ZEROBUILD_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.zerobuild" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official ZeroBuild image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.zerobuild:/zerobuild-data/.zerobuild"
  workspace_mount="$docker_data_dir/workspace:/zerobuild-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  ZEROBUILD_API_KEY="sk-..." ./zerobuild_install.sh --docker
or run interactive:
  ./zerobuild_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
...
CONTAINER_CLI="${ZEROBUILD_CONTAINER_CLI:-docker}"
API_KEY="${ZEROBUILD_API_KEY:-}"
PROVIDER="${ZEROBUILD_PROVIDER:-openrouter}"
MODEL="${ZEROBUILD_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${ZEROBUILD_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set ZEROBUILD_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t zerobuild-bootstrap-XXXXXX)"
...
Your containerized ZeroBuild data is persisted under:
DONE
...
Next steps:
  ./zerobuild_install.sh --docker --interactive-onboard
  ./zerobuild_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./zerobuild_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing zerobuild to cargo bin"
...
ZEROBUILD_BIN=""
if have_cmd zerobuild; then
  ZEROBUILD_BIN="zerobuild"
elif [[ -x "$HOME/.cargo/bin/zerobuild" ]]; then
  ZEROBUILD_BIN="$HOME/.cargo/bin/zerobuild"
elif [[ -x "$WORK_DIR/target/release/zerobuild" ]]; then
  ZEROBUILD_BIN="$WORK_DIR/target/release/zerobuild"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$ZEROBUILD_BIN" ]]; then
    error "onboarding requested but zerobuild binary is not available."
    error "Run without --skip-install, or ensure zerobuild is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$ZEROBUILD_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  ZEROBUILD_API_KEY="sk-..." ./zerobuild_install.sh --onboard
or run interactive:
  ./zerobuild_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$ZEROBUILD_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
...
Next steps:
  zerobuild status
  zerobuild agent -m "Hello, ZeroBuild!"
  zerobuild gateway
DONE
