# ZeroBuild Security Improvement Roadmap
...
ZeroBuild already has **excellent application-layer security**:
...
## Comparison: ZeroBuild vs PicoClaw vs Production Grade

| Feature | PicoClaw | ZeroBuild Now | ZeroBuild + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`zerobuild audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/zerobuild/audit.log"
sign_events = true
max_size_mb = 100
...
zerobuild security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
zerobuild audit --user @alice --since 24h
zerobuild audit --risk high --violations-only
zerobuild audit --verify-signatures

# Sandbox test
zerobuild sandbox --test
# → Testing isolation...
...
**ZeroBuild is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, ZeroBuild becomes:
...
**Value**: Transforms ZeroBuild from "safe for testing" to "safe for production"
