# Adding Boards and Tools — ZeroBuild Hardware Guide

This guide explains how to add new hardware boards and custom tools to ZeroBuild.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.zerobuild/config.toml)
zerobuild peripheral add nucleo-f401re /dev/ttyACM0
zerobuild peripheral add arduino-uno /dev/cu.usbmodem12345
zerobuild peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, ZeroBuild can index PDF files:
...
2. **Add to config** — `zerobuild peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `zerobuild peripheral list` | List configured boards |
| `zerobuild peripheral add <board> <path>` | Add board (writes config) |
| `zerobuild peripheral flash` | Flash Arduino firmware |
| `zerobuild peripheral flash-nucleo` | Flash Nucleo firmware |
| `zerobuild hardware discover` | List USB devices |
| `zerobuild hardware info` | Chip info via probe-rs |
