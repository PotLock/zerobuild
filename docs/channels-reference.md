...
This document is the canonical reference for channel configuration in ZeroBuild.
...
6. **Stale runtime state**: config changed but `zerobuild daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.zerobuild/config.toml`.
...
When running `zerobuild channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `zerobuild models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

ZeroBuild supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `zerobuild channel list`, `zerobuild channel doctor`, and `zerobuild channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@zerobuild:matrix.example.com"   # optional, recommended for E2EE
...
ZeroBuild supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.zerobuild/state/whatsapp-web/session.db"
...
port = 6697
nickname = "zerobuild-bot"
username = "zerobuild"              # optional
channels = ["#zerobuild"]
allowed_users = ["*"]
...
- `ZEROBUILD_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `ZEROBUILD_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `zerobuild daemon` after config changes.
...
```bash
RUST_LOG=info zerobuild daemon 2>&1 | tee /tmp/zerobuild.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/zerobuild.log
```
