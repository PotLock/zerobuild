...
3. Generic fallback env vars: `ZEROBUILD_API_KEY` then `API_KEY`
...
- After multimodal normalization, ZeroBuild sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, ZeroBuild returns a structured capability error instead of silently ignoring images.
...
- ZeroBuild normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `zerobuild models refresh --provider llamacpp`
...
- Model discovery: `zerobuild models refresh --provider sglang`
...
- Model discovery: `zerobuild models refresh --provider vllm`
...
- Model discovery: `zerobuild models refresh --provider osaurus`
...
- Model discovery: `zerobuild models refresh --provider nvidia`
