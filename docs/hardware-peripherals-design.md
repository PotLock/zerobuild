# Hardware Peripherals Design — ZeroBuild

ZeroBuild enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** ZeroBuild acts as a hardware-aware AI agent that:
...
**Mental model:** ZeroBuild = brain that understands hardware. Peripherals = arms and legs it controls.
...
ZeroBuild runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  ZeroBuild on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. ZeroBuild fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
ZeroBuild runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  ZeroBuild on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. ZeroBuild identifies connected hardware (VID/PID, architecture)
...
2. ZeroBuild writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. ZeroBuild auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| ZeroBuild runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs ZeroBuild; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

ZeroBuild on Pi; GPIO via rppal or sysfs. No separate firmware.
...
board = "esp32"
transport = "wifi"
# Edge-Native: ZeroBuild runs on ESP32
```
...
1. **Startup:** ZeroBuild loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between ZeroBuild and peripherals:
...
- **zerobuild-firmware** or **zerobuild-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; ZeroBuild connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`zerobuild peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `zerobuild hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `zerobuild hardware introspect <path>`: memory map, peripheral list
...
- [x] ZeroBuild on Raspberry Pi (native GPIO via rppal)
...
- [x] `zerobuild-esp32` firmware crate (`firmware/zerobuild-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] ZeroBuild *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/zerobuild-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full ZeroBuild *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). ZeroBuild runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. ZeroBuild gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: ZeroBuild from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey ZeroBuild, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
