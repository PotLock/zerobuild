...
- `cargo check` / `cargo build` appears stuck at `Checking zerobuild` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in ZeroBuild:
...
### `zerobuild` command not found after install

Symptom:

- install succeeds but shell cannot find `zerobuild`
...
- stop extra `zerobuild daemon` / `zerobuild channel start` processes
