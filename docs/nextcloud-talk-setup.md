...
This guide covers native Nextcloud Talk integration for ZeroBuild.
...
Add this section in `~/.zerobuild/config.toml`:
...
zerobuild daemon
# or
zerobuild gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, ZeroBuild verifies:
...
- ZeroBuild ignores bot-originated webhook events (`actorType = bots`).
- ZeroBuild ignores non-message/system events.
...
3. Confirm ZeroBuild receives and replies in the same room.
