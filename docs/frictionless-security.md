...
[INFO] ✓ Audit logging enabled (~/.config/zerobuild/audit.log)
...
```bash
# Check what's active
$ zerobuild security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/zerobuild/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ zerobuild config set security.sandbox.enabled false

# Enable specific backend
$ zerobuild config set security.sandbox.backend firejail

# Adjust limits
$ zerobuild config set security.resources.max_memory_mb 2048
```
...
```bash
$ zerobuild onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `zerobuild security --status`
