...
ZeroBuild supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export ZEROBUILD_API_KEY="your-api-key"
zerobuild agent
```

## llama.cpp Server (Recommended Local Setup)

ZeroBuild includes a first-class local provider for `llama-server`:
...
You do not need to export `ZEROBUILD_API_KEY=dummy` for this flow.

## SGLang Server

ZeroBuild includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
ZeroBuild includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
# Single message test
zerobuild agent -m "test message"
```
