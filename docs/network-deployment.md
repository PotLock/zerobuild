# Network Deployment — ZeroBuild on Raspberry Pi and Local Network

This document covers deploying ZeroBuild on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | ZeroBuild polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | ZeroBuild syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — ZeroBuild connects to external servers/relays. No port forwarding or public IP required.

---

## 2. ZeroBuild on Raspberry Pi
...
```bash
zerobuild daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: ZeroBuild polls Telegram API (outbound)
...
```bash
zerobuild daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `zerobuild tunnel` (see tunnel docs).

3. ZeroBuild will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- ZeroBuild calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `zerobuild daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `zerobuild daemon` service).
...
- [ ] Run `zerobuild daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
ZeroBuild supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `zerobuild` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo zerobuild service install
```

This creates:
- Init script: `/etc/init.d/zerobuild`
- Config directory: `/etc/zerobuild/`
- Log directory: `/var/log/zerobuild/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo zerobuild service install` automatically prepares `/etc/zerobuild`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `zerobuild` service user.

If no prior runtime state is available to migrate, create `/etc/zerobuild/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add zerobuild default

# Start the service
sudo rc-service zerobuild start

# Check status
sudo rc-service zerobuild status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service zerobuild start` | Start the daemon |
| `sudo rc-service zerobuild stop` | Stop the daemon |
| `sudo rc-service zerobuild status` | Check service status |
| `sudo rc-service zerobuild restart` | Restart the daemon |
| `sudo zerobuild service status` | ZeroBuild status wrapper (uses `/etc/zerobuild` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/zerobuild/access.log` |
| Errors/stderr | `/var/log/zerobuild/error.log` |

View logs:

```bash
sudo tail -f /var/log/zerobuild/error.log
```
...
sudo rc-service zerobuild stop
sudo rc-update del zerobuild default

# Remove init script
sudo zerobuild service uninstall
```
...
- The service runs as the `zerobuild:zerobuild` user (least privilege)
- Config must be at `/etc/zerobuild/config.toml` (explicit path in init script)
- If the `zerobuild` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo zerobuild service install`
- [ ] Enable: `sudo rc-update add zerobuild default`
- [ ] Start: `sudo rc-service zerobuild start`
- [ ] Verify: `sudo rc-service zerobuild status`
- [ ] Check logs: `/var/log/zerobuild/error.log`
