...
Default behavior is **app-only** (build/install ZeroBuild) and expects existing Rust toolchain.
...
This builds a local ZeroBuild image and launches onboarding inside a container while
...
```bash
ZEROBUILD_API_KEY="sk-..." ZEROBUILD_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
