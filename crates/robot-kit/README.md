...
A complete toolkit for building AI-powered robots with ZeroBuild. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build ZeroBuild with robot tools

```bash
# Clone and build
git clone https://github.com/zerobuild-labs/zerobuild
cd zerobuild
cargo build -p zerobuild-robot-kit --release
```
...
# Copy config
mkdir -p ~/.zerobuild
cp crates/robot-kit/robot.toml ~/.zerobuild/
cp crates/robot-kit/SOUL.md ~/.zerobuild/workspace/
...
# Test in mock mode
./target/release/zerobuild agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/zerobuild agent -m "Move forward 1 meter"
```
...
```rust
use zerobuild_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `zerobuild`, add a thin adapter that maps this
...
mkdir zerobuild-robot-kit
cp -r target/release/zerobuild zerobuild-robot-kit/
cp -r examples/robot_kit zerobuild-robot-kit/
cp -r ~/.zerobuild zerobuild-robot-kit/dot-zerobuild

# Include models
mkdir -p zerobuild-robot-kit/models
cp ~/.zerobuild/models/ggml-base.bin zerobuild-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf zerobuild-robot-kit.tar.gz zerobuild-robot-kit/

# Copy to USB
cp zerobuild-robot-kit.tar.gz /media/usb/TarBalls/
```
