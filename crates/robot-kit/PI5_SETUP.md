...
Complete guide to setting up a ZeroBuild-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.zerobuild/models
cp models/ggml-base.bin ~/.zerobuild/models/
```
...
mkdir -p ~/.zerobuild/models/piper
cd ~/.zerobuild/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.zerobuild/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build ZeroBuild Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/zerobuild
cd zerobuild

# Build robot kit
cargo build --release -p zerobuild-robot-kit

# Build main zerobuild (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.zerobuild
nano ~/.zerobuild/robot.toml
```

```toml
# ~/.zerobuild/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.zerobuild/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/zerobuild_sensors.fifo"
...
### Start ZeroBuild Agent

```bash
# Configure ZeroBuild to use robot tools
cat > ~/.zerobuild/config.toml << 'EOF'
...
cp ~/zerobuild/crates/robot-kit/SOUL.md ~/.zerobuild/workspace/

# Start agent
./target/release/zerobuild agent
```
...
# Start sensor loop
if [ ! -p /tmp/zerobuild_sensors.fifo ]; then
    mkfifo /tmp/zerobuild_sensors.fifo
fi
...
```bash
# /etc/systemd/system/zerobuild-robot.service
sudo tee /etc/systemd/system/zerobuild-robot.service << 'EOF'
[Unit]
Description=ZeroBuild Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/zerobuild
...
sudo systemctl enable zerobuild-robot
sudo systemctl start zerobuild-robot

# Check status
sudo systemctl status zerobuild-robot
journalctl -u zerobuild-robot -f  # View logs
```
